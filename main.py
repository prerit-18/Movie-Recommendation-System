import os
# Used to interact with the operating system (file paths, environment variables, folders, etc.)

import pickle
# Used to load and save trained ML models or Python objects as binary files

from typing import Optional, List, Dict, Any, Tuple
# Used for type hints (better code readability and validation)
...
import numpy as np
# Used for numerical operations, arrays, matrix computations (very common in ML)

import pandas as pd
# Used for handling datasets (DataFrames, CSV files, data cleaning)

import httpx
# Used to make HTTP requests (like calling external APIs, e.g., movie poster API)

from fastapi import FastAPI, HTTPException, Query
# FastAPI → creates the web application
# HTTPException → used to return API errors (like 404 not found)
# Query → used to define query parameters in API endpoints

from fastapi.middleware.cors import CORSMiddleware
# Allows frontend (React, etc.) to connect to backend without CORS errors

from pydantic import BaseModel
# Used to define request/response data models with validation

from dotenv import load_dotenv
# Loads environment variables from a .env file (for API keys, secrets, etc.)
